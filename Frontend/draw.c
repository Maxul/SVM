...
//printf("%d\n", *(int *)(mem + 2 + sizeof(int) * 6 + 200));
...
#ifdef _DEBUG
            printf("Win 0x%lx\t%lldX\t", guest_wid, iii++);
            printf("size: %lf MB\t", (double)malloc_usable_size((void *)(image_tmp->data))/1024/1024);
            printf("time: %ld us\n", clock() - now);
            now = clock();
#endif
        ...
        if ('-' == mem[0] && GetSize(pq) > 0)
        {
            Item item_tmp;
            DeQueue(pq, &item_tmp);
            memcpy(mem + KMEM_SIZE + 20, (void *)&item_tmp, sizeof(item_tmp));
